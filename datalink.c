...
{
    *(unsigned int *)(frame + len) = crc32(frame, len); // crc 译码
    ...
    PACKET buffer[])
{
    ...
    case FRAME_DATA: // 发送数据帧，数据帧长度为 2
        ...
        start_timer(seq % CACHE_SIZE, DATA_TIMEOUT_LMT); // 发送数据帧后的重传
        计时
        break;
    case FRAME_ACK:
        dbg_frame("Send ACK %d\n", frame.ack); // 发送 ACK 应答帧，ACK 帧长度为 2
        putFrame((byte *)&frame, 2);
        break;
    case FRAME_NAK:
        noNAK = false;
        dbg_frame("Send NAK %d\n", frame.ack); // 发送 NAK 重传指示帧，NAK 帧长度
        为 2 putFrame((byte *)&frame, 2);
        break;
    }
    stop_ack_timer();
}
...
int main(int argc, char *argv[])
{
    // 数据定义
    static byte
        to_send_frame = 0,    // 发送窗口上界
        expACK = 0,           // 发送窗口下界
        too_Far = CACHE_SIZE, // 接收窗口上界
        expFrame = 0;         // 接收窗口下界
    static PACKET
        oBuf[CACHE_SIZE],               // 输出暂存区
        iBuf[CACHE_SIZE];               // 输入暂存区
    ...
    for (;;)
    {
        ...
        switch (event)
        {
        case NETWORK_LAYER_READY: // 网络层就绪，接受包并将数据帧传递至返回
            区
                nbuffered++;
        ...
        case FRAME_RECEIVED: // 接受帧
            ...
            case FRAME_DATA:
                ...
                if (between(expFrame, frame.seq, too_Far) && (arrived[frame.seq %
                                                                      CACHE_SIZE] == 0))
                {
                ...
                break;
            case FRAME_NAK:
                if (between(expACK, (frame.ack + 1) % (MAX_SEQ + 1),
                            to_send_frame))
                {
